...
- **Requires Craft CMS 6 and PHP 8.5.** Craft CMS 5 is maintained on the `5.x`
  branch.
...
- **The package is `quebecstudio-mods/craftcms-consent-kit`**, and its namespace
  `QuebecStudioMods\ConsentKit\CraftCms`. The plugin handle and the settings are unchanged.
- **Versions follow Craft CMS majors**: 5.x for Craft CMS 5, 6.x for Craft CMS 6.
  Entries below 5.0.0 use the previous numbering.
- Native Craft CMS 6 plugin, without the Yii adapter.
- The core, the templates and the front-end assets come from the
  `quebecstudio-mods/consent-kit-core` package.
- The config file is `config/craft/cookie-consent-kit.php`.
  `config/cookie-consent-kit.php` is still read, and logs a deprecation.
- **Visitor-facing wording comes from language files**, per language:
  shipped with the core package, replaced or extended by the project's
  `lang/vendor/cookie-consent-kit/<language>.php`.
- The shipped templates are Blade. Site overrides in the `templateRoot` folder
  may be Twig or Blade.
- The settings screens are built with Craft's control panel forms: seven panes,
  per-site editing through site links, Copy from on the Cookie inventory pane,
  several categories removable in one save.

### Added

- `$craft->consent()` in Blade templates, with the same methods as
  `craft.consent`.
...
- The `withCookieTable` Twig filter stays registered when request-scoped
  services are flushed, as under Octane.
